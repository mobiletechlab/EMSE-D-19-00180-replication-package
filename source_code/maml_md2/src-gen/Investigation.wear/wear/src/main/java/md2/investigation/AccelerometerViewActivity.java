...
import md2.investigation.md2.controller.action.Accelerometer_Accelerometer_done_Action;

import de.wwu.md2.android.md2library.SensorHelper;


public class AccelerometerViewActivity extends WearableActivity implements WearableActionDrawer.OnMenuItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		drawerLayout = (WearableDrawerLayout) findViewById(R.id.drawer_layout_AccelerometerView);
		drawerLayout.setDrawerStateCallback(new WearableDrawerLayout.DrawerStateCallback() {
			...
			public void onDrawerClosed(View view) {
				if(adapter.close()){
					AccelerometerViewActivity.this.finish();
				}
			}
			@Override
...
if(actionDrawer.isPeeking()){
	...
	handler.postDelayed(new Runnable() {
		...
		navigationDrawer = (WearableNavigationDrawer) findViewById(R.id.navigation_drawer_AccelerometerView);
		adapter = NavigationAdapter.getInstance();
		navigationDrawer.setAdapter(adapter);

		actionDrawer = (WearableActionDrawer) findViewById(R.id.bottom_action_drawer_AccelerometerView);
	...
	public boolean onMenuItemClick(MenuItem menuItem) {
		...
		switch(itemId) {
			case R.id.AccelerometerView_item0:
				ca = new Accelerometer_Accelerometer_getAccelerometerValue_Action();
				break;

			case R.id.AccelerometerView_item1:
				ca = new Accelerometer_Accelerometer_done_Action();
				break;
