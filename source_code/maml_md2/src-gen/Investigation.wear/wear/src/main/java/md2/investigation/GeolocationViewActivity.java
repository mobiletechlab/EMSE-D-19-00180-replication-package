...
import md2.investigation.md2.controller.action.Geolocation_Geolocation_done_Action;

import de.wwu.md2.android.md2library.SensorHelper;


public class GeolocationViewActivity extends WearableActivity implements WearableActionDrawer.OnMenuItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		drawerLayout = (WearableDrawerLayout) findViewById(R.id.drawer_layout_GeolocationView);
		drawerLayout.setDrawerStateCallback(new WearableDrawerLayout.DrawerStateCallback() {
			...
			public void onDrawerClosed(View view) {
				if(adapter.close()){
					GeolocationViewActivity.this.finish();
				}
			}
			@Override
...
if(actionDrawer.isPeeking()){
	...
	handler.postDelayed(new Runnable() {
		...
		navigationDrawer = (WearableNavigationDrawer) findViewById(R.id.navigation_drawer_GeolocationView);
		adapter = NavigationAdapter.getInstance();
		navigationDrawer.setAdapter(adapter);

		actionDrawer = (WearableActionDrawer) findViewById(R.id.bottom_action_drawer_GeolocationView);
	...
	public boolean onMenuItemClick(MenuItem menuItem) {
		...
		switch(itemId) {
			case R.id.GeolocationView_item0:
				ca = new Geolocation_Geolocation_getGeolocationValue_Action();
				break;

			case R.id.GeolocationView_item1:
				ca = new Geolocation_Geolocation_done_Action();
				break;
