...
public class AccelerometerProvider extends AbstractMd2ContentProvider {

	public AccelerometerProvider(String key, Md2Entity content, Md2DataStore md2DataStore) {
	...
	public Md2Type getValue(String attribute) {			
		switch (attribute){
			case "accelerometerValueX":
				if(((InvestigationAccelerometer)content).getAccelerometerValueX() != null){
					return  
						new Md2Float
						(((InvestigationAccelerometer)content).getAccelerometerValueX());	
				} else { return null;}
			case "accelerometerValueY":
				if(((InvestigationAccelerometer)content).getAccelerometerValueY() != null){
					return  
						new Md2Float
						(((InvestigationAccelerometer)content).getAccelerometerValueY());	
				} else { return null;}
			case "accelerometerValueZ":
				if(((InvestigationAccelerometer)content).getAccelerometerValueZ() != null){
					return  
						new Md2Float
						(((InvestigationAccelerometer)content).getAccelerometerValueZ());	
				} else { return null;}
			default: return null;
		}
	}
		...
		if ((this.getValue(name) == null && value != null) || value != null && !this.getValue(name).toString().equals(value.toString())) {
			switch (name){
				case "accelerometerValueX":
					...
					((InvestigationAccelerometer)content).setAccelerometerValueX(((Md2Float) value).getPlatformValue());
break; // Avoid errors for unsupported type
				case "accelerometerValueY":
					// Umgang mit anderen Datentypen hier einfügen - derzeit kein Support für Listen innerhalb v. Entities
					// angenommen wird entweder Md2String oder passender Md2Type als value
					((InvestigationAccelerometer)content).setAccelerometerValueY(((Md2Float) value).getPlatformValue());
break; // Avoid errors for unsupported type
				case "accelerometerValueZ":
					// Umgang mit anderen Datentypen hier einfügen - derzeit kein Support für Listen innerhalb v. Entities
					// angenommen wird entweder Md2String oder passender Md2Type als value
					((InvestigationAccelerometer)content).setAccelerometerValueZ(((Md2Float) value).getPlatformValue());
break; // Avoid errors for unsupported type
			}
		}
	}
