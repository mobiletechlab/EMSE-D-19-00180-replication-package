...
public class Geolocation_Geolocation_getGeolocationValue_Action extends AbstractMd2Action {

    public Geolocation_Geolocation_getGeolocationValue_Action() {
		super("Geolocation_Geolocation_getGeolocationValue_Action"); 
	}

    @Override
    public void execute() {
		LocationProvider.callback = this;

		Md2ContentProviderRegistry cpr = Md2ContentProviderRegistry.getInstance();
		Md2AttributeSetTask var1 = null;
		try {

			var1 = new Md2AttributeSetTask("GeolocationProvider", "locationLong", cpr.getContentProvider("LocationProvider").getValue("locationLong"));
			var1.execute();
		...
		try {
			var2 = new Md2AttributeSetTask("GeolocationProvider", "locationLat", cpr.getContentProvider("LocationProvider").getValue("locationLat"));
			var1.execute();
			var2.execute();
		}catch (Md2WidgetNotCreatedException e){
	...
	public void callback(Location location){
		...
		EinkaufszettelApp.getBenchmarkFragment().onBenchmarkCompleted(location.getLongitude() + " " + location.getLatitude());
	}
}
