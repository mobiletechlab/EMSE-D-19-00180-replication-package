...
public class Camera___Camera_processChainExecuteStepAction_Action extends AbstractMd2Action {

    public Camera___Camera_processChainExecuteStepAction_Action() {
		super("Camera___Camera_processChainExecuteStepAction_Action"); 
	}
    ...
    public void execute() {
		...
		if((((Md2String) Md2ContentProviderRegistry.getInstance().getContentProvider("__processChainControllerStateProvider").getValue("currentProcessChainStep")).equals(new Md2String("firstProcessChain__firstStep")))){
			...
			try {
				var1 = new Md2CallTask(new Md2GoToViewAction(EinkaufszettelApp.getAppContext().getString(R.string.CameraViewActivity)));
				var1.execute();
